...
def run_single_test(B, M, N, K, chunk=False, dtype=torch.bfloat16):
    ...
    if chunk:
        ...
        ref_q = torch.randn(M, B, N + 64, dtype=dtype).narrow(2, 0, N).transpose_(0, 1)
    else:
    ...
    ref_q.zero_()
    assert ref_q.sum().item() == 0
    native_bmm_fp8(matA, matB_q, matB_s, ref_q)
    print(ref_q[0][0][0])
    
    matC.zero_()
    assert matC.sum().item() == 0    
    from flashinfer import _kernels
    matB_q = matB_q.transpose(1, 2).contiguous()
    _kernels.bmm(matC, matA, matB_q, False, matB_s)
    print("my fp8 flashinfer compare")
    compare(ref_q, matC)
    print(ref[0][0][0])
    ...
    sgl_kernel.cpu.fp8_scaled_mm(
        matC, matA, matB_q, is_vnni=False, scales2=matB_s
    )
    print("my fp8 sgl-kernel compare")
    compare(ref_q, matC)
    print(ref[0][0][0])
